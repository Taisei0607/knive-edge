...
# one step is 0.02 mm, voltage 12 V
...
# setting of sweep
# sweep_total_steps = 3000 # total number of steps ini:6000
# step_increment = 150 # number of steps in a measurement interval ini:150 (b3), 75 (b6)
step_delay = 0.0005 # step delay of motor (speed of motor)
# measurement_delay = 2 # initial delay ini:2 (b3), 4 (b6)

port_mapping = {'port1': 'Band6_USB', 'port2': 'Band6_LSB', 'port3': 'Band3_USB', 'port4': 'Band3_LSB'}
direction = input('True? (y/n): ').lower()
is_clockwise = (direction == 'y')
sweep_total_steps = int(input('sweep total steps: ')) # 6000
step_increment = int(input('step incremet: ')) # 150 = 3 mm
measurement_delay = int(input('measurement delay: ')) # 2
aperture_time = int(input('sensor aperture time (0.01 ms ~ 300 ms): '))
sensor1_port = input('sensor1: ')
sensor2_port = input('sensor2: ')
sensor1_label = port_mapping.get(sensor1_port, sensor1_port)
sensor2_label = port_mapping.get(sensor2_port, sensor2_port)
...
for i in[sensor1, sensor2]:
    i.initialize(freq_ghz = 13, avg_count = int(2**6), aperture_ms = aperture_time)
    i.start()
...
for i in range(0, sweep_total_steps, step_increment):
    ...
    motor.motor_go(is_clockwise, # True = Clockwise, False = Counter-Clockwise
...
# first way
if result['steps']:
    timestamp_day = time.strftime('%Y%m%d') # include the current date and time
    timestamp_time = time.strftime('%H%M%S')
    save_path = f'/home/raspi/Desktop/{timestamp_day}' # set the desktop path
    
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    
    fig, ax = plt.subplots(1, 2, figsize = (16, 8))
    ax[0].plot(result['steps'], result['power1'], label = sensor1_label)
    ax[0].set_title(f'knive edge double {timestamp_day}_{timestamp_time}\n{sweep_total_steps} steps at {step_increment} step intervals')
    ax[0].set_xlabel(f'position ({steps_per_rev} steps per one rotation) [mm]')
    ax[0].set_ylabel('power intensity [dBm]')
    ax[0].legend()
    ax[0].grid(True)
    ax[1].plot(result['steps'], result['power2'], label = sensor2_label)
    ax[1].set_title(f'knive edge double {timestamp_day}_{timestamp_time}\n{sweep_total_steps} steps at {step_increment} step intervals')
    ax[1].set_xlabel(f'position ({steps_per_rev} steps per one rotation) [mm]')
    ax[1].set_ylabel('power intensity [dBm]')
    ax[1].legend()
    ax[1].grid(True)
    
    filename_img = os.path.join(save_path, f"power_sweep_graph_double_{timestamp_day}_{timestamp_time}.png")
    filename_csv = os.path.join(save_path, f"power_sweep_data_double_{timestamp_day}_{timestamp_time}.csv")
...
script_basename = 'knive_edge_analysis_double.py'
script = os.path.join(os.path.dirname(__file__), script_basename)
cmd = 'python3 %s %s'%(script, filename_csv)
os.system(cmd)
...
#    filename_csv = os.path.join(save_path, f"power_sweep_data_double_{timestamp}.csv")
...
#    plt.title(f"knive_edge_single_{timestamp}")
...
#    filename_img = os.path.join(save_path, f"dual_power_graph_double_{timestamp}.png")
#    plt.savefig(filename_img)
