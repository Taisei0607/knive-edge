...
COM_PORT = "/dev/ttyACM0" # find by hitting "ls /dev/tty*"
BAUDRATE = 19200

# decrare a instance of power sensor
try:
    sensor = PowerSensor(port = COM_PORT, baudrate = BAUDRATE)
except serial.SerialException:
    print("Error: Unable to connect to power sensor. Please check comport.")
    exit()
...
for i in range(0, sweep_total_steps, step_increment):
    ...
    # measure power
    power_value = sensor.get_power()
    
    if power_value is not None:
        result['steps'].append(current_motor_position)
        result['power'].append(power_value)
        print(f"position: {(current_motor_position):4d} steps | power: {power_value:.3f} dBm")
    else:
...
if result['steps']:
    ...
    save_path = f'/home/raspi/Desktop/{timestamp_day}'
    
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    
    plt.figure(figsize = (10, 6))
    plt.plot(result['steps'], result['power'])
    plt.title(f"knive_edge_single_{timestamp_day}_{timestamp_time}")
#   plt.title('relation between position and power intensity')
    ...
    filename_img = os.path.join(save_path, f"power_sweep_graph_single_{timestamp_day}_{timestamp_time}.png")
    filename_csv = os.path.join(save_path, f"power_sweep_data_single_{timestamp_day}_{timestamp_time}.csv")
    data_to_save = np.stack([result['steps'], result['power']], axis = 1) # prepare the data array
